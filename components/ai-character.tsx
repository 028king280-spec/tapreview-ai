  ...
  return (
    <motion.div animate={{ y: [0, -6, 0], rotate: thinking ? [-2, 2, -2] : 0 }} transition={{ duration: thinking ? 1.3 : 4, repeat: Infinity, ease: 'easeInOut' }} className="relative mx-auto flex size-48 items-center justify-center sm:size-56" role="img" aria-label={`Milo: ${stateCopy[state]}`}>
      ...
      <div className="relative z-10 mt-2 flex flex-col items-center gap-6">
        ...
        <motion.div animate={{ width: concerned ? 25 : happy ? 38 : 23, height: speaking ? [4, 15, 4, 12, 4] : concerned ? 2 : happy ? 15 : 3 }} transition={{ duration: speaking ? .65 : .25, repeat: speaking ? Infinity : 0 }} className="rounded-b-full border-b-4 border-[#183b34]" />
      </div>
      <motion.div animate={{ opacity: ['listening', 'speaking'].includes(state) ? [0.2, 1, .2] : 0, scale: ['listening', 'speaking'].includes(state) ? [1, 1.05, 1] : 1 }} transition={{ duration: 1.2, repeat: Infinity }} className="absolute -inset-4 rounded-[44%] border-2 border-[#7ca78f]" />
...
export function Waveform({ tone = 'light' }: { tone?: 'light' | 'dark' }) {
  const color = tone === 'light' ? '#fff8ed' : '#dc7251'
  return <div className="flex h-8 items-center justify-center gap-1" aria-hidden="true">{[3, 8, 14, 21, 11, 6, 16, 9, 4].map((height, index) => <motion.span key={index} animate={{ height: [height, height + 8, height] }} transition={{ duration: .75, repeat: Infinity, delay: index * .08 }} className="w-1 rounded-full" style={{ height, backgroundColor: color }} />)}</div>
}
